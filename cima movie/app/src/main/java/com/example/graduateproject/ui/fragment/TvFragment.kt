...
import com.example.graduateproject.adapters.CardSeriesSAdapter
...
class TvFragment : Fragment() {
    ...
    private val mAdapterSeries by lazy { CardSeriesSAdapter() }
    ...
    override fun onCreateView(
        ...
        observeAllData()
        mview.recy_Tvfrag.addOnScrollListener(object : RecyclerView.OnScrollListener() {
    ...
    fun observeAllData() {
        dataViewModel.getSeries(applyViewModel.appluQueri(currentPage.toString()))
        dataViewModel.series.observe(viewLifecycleOwner, {
            if (it is StatusResult.OnSuccess) {
                ...
                showShimmer(false, mview.recy_Tvfrag)
            }
            if (it is StatusResult.OnLoading) {
                showShimmer(true, mview.recy_Tvfrag)
            }
